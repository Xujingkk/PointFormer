...
def draw_loss(train_loss, val_loss, now):
    ...
    plt.xlabel('epoch')
    plt.savefig('/root/autodl-tmp/transnuseg/log/TrainLoss_{}.png'.format(now), dpi=100)
    plt.show()
...
def _cia_loss(pred, target, w):
    ...
    for i in range(0, b):
        # compute the IoU of the foreground
        classes = target[i] > 0
        Iand1 = -torch.sum(classes * torch.log(pred[i][0] + 1e-6) / (torch.sum(classes) + 1) + ~classes * torch.log(
            1 - pred[i][0] + 1e-6) / (torch.sum(~classes) + 1))
        # print('class{}: {}'.format(j,Iand1))
        IoU = IoU + (1 - w) * Iand1

        classes = target[i] > 1
        Iand1 = -torch.sum(classes * torch.log(pred[i][1] + 1e-6) / (torch.sum(classes) + 1) + ~classes * torch.log(
            1 - pred[i][1] + 1e-6) / (torch.sum(~classes) + 1))
...
def _st_loss(pred, target, thresh):
    ...
    for i in range(0, b):
        ...
        certain = pt > thresh
        Iand1 = -(
            torch.sum(certain * torch.log(pt + 1e-6) + ~certain * (np.log(thresh) - (1 - (pt / thresh) ** 2) / 2)))
        ST = ST + Iand1 / 512 / 512
    # print("_st",ST/b)
    return ST / b
...
def get_fast_aji(true, pred):
    ...
    print("true", true_id_list)
    pred_id_list = list(np.unique(pred))
    print("pred", pred_id_list)
    if len(true_id_list) == 1 and true_id_list[0] == 0:
        aji_score = np.nan
    else:
        true_masks = [
            None,
        ...
        for true_id in true_id_list[1:]:  # 0-th is background
            ...
            # print("pred_true_overlap_id", pred_true_overlap_id)
        ...
        if len(pairwise_iou) == 0 or len(pairwise_iou[0]) == 0:
            aji_score = np.nan
        else:
            # print("pairwise_iou",pairwise_iou.shape)
...
def get_fast_pq(true, pred, match_iou=0.5):
    ...
    pred = np.copy(pred)
    true_id_list = list(np.unique(true))
    # print("true_id_list", true_id_list)
    pred_id_list = list(np.unique(pred))
    ...
    if len(true_id_list) == 1 and true_id_list[0] == 0:
        ...
        sq = np.nan
        pq = np.nan
    elif len(true_id_list) == 1 and true_id_list[0] == 1:
        dq = np.nan
        sq = np.nan
        pq = np.nan
