...
from model.MLPFormer import DualTransformerBlock, Cross_DualTransformerBlock
...
class CrossWindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None, cross_q=None):
        """
        Args:
            x: input features with shape of (num_windows*B, N, C)
            mask: (0/-inf) mask with shape of (num_windows, Wh*Ww, Wh*Ww) or None
        ...
        # cross_qkv = self.cross_qkv(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)
        # cross_q, cross_k, cross_v = cross_qkv[0], cross_qkv[1], cross_qkv[2]
...
class Cross_SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, qkv, window_size=7, shift_size=0,
    ...
    def forward(self, x, cross_q=None):
        ...
        x_windows = x_windows.view(-1, self.window_size * self.window_size, C)  # nW*B, window_size*window_size, C
        # print("x_windows1", x_windows.shape)

        # W-MSA/SW-MSA
        attn_windows, q = self.attn(x_windows, mask=self.attn_mask, cross_q=cross_q)  # nW*B, window_size*window_size, C
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, split_size, window_size,
...
class xjCross_BasicLayer_up(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, split_size, window_size,
        ...
        block = []
        for i in range(depth):
            mlpblock = Cross_DualTransformerBlock(dim=dim, resolution=input_resolution,
                                 heads=num_heads,
                                 split_size=split_size
                                                   )
            block.append(mlpblock)
        self.blocks = nn.ModuleList(block)
...
class PointFormer(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=3, num_classes=2, nuc_classes=5,
                 embed_dim=96, depths=[2, 2, 2, 2], depths_decoder=[1, 2, 2, 2], num_heads=[3, 6, 12, 24],
                 window_size=8, mlp_ratio=4., qkv_bias=True, qk_scale=None, split_sizes=[1, 2, 4, 8],
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer == 0:
                ...
                layer_up = xjCross_BasicLayer_up(dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                               input_resolution=(
                                               ...
                                               use_checkpoint=use_checkpoint)
                layer_up2 = xjCross_BasicLayer_up(dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                                input_resolution=(
                                                    patches_resolution[0] // (2 ** (self.num_layers - 1 - i_layer)),
                                                    patches_resolution[1] // (2 ** (self.num_layers - 1 - i_layer))),
                                                depth=depths[(self.num_layers - 1 - i_layer)],
                                                ...
                                                upsample=PatchExpand if (i_layer < self.num_layers - 1) else None,
                                                use_checkpoint=use_checkpoint)
                layer_up3 = xjCross_BasicLayer_up(dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                                input_resolution=(
                                                    patches_resolution[0] // (2 ** (self.num_layers - 1 - i_layer)),
                                                    patches_resolution[1] // (2 ** (self.num_layers - 1 - i_layer))),
                                                depth=depths[(self.num_layers - 1 - i_layer)],
            ...
            self.layers_up3.append(layer_up3)  ## edge decoders sharing parts of blocks
        ...
        if self.final_upsample == "expand_first":
            ...
            self.up3 = FinalPatchExpand_X4(input_resolution=(img_size // patch_size, img_size // patch_size),
                                           dim_scale=4, dim=embed_dim)
            self.output3 = nn.Conv2d(in_channels=embed_dim, out_channels=self.nuc_classes, kernel_size=1, bias=False)
    ...
    def forward_features(self, x):
        ...
        point_mask = self.pos_drop(x)

        seg_mask_downsample = []
        edge_mask_downsample = []
        point_mask_downsample = []
        # print("seg_mask0", seg_mask.shape)

        for layer in self.layers:
            edge_mask_downsample.append(edge_mask)
            seg_mask_downsample.append(seg_mask)
            point_mask_downsample.append(point_mask)

            seg_mask = layer(seg_mask)
            edge_mask = layer(edge_mask)
            point_mask = layer(point_mask)

        seg_mask = self.norm(seg_mask)  # B L C
        edge_mask = self.norm(edge_mask)
        point_mask = self.norm(point_mask)

        # print("seg_mask_downsample", seg_mask_downsample[0].shape, seg_mask_downsample[1].shape)

        return seg_mask, edge_mask, point_mask, seg_mask_downsample, edge_mask_downsample, point_mask_downsample

    # Dencoder and Skip connection
    def forward_up_features(self, seg_mask, edge_mask, point_mask, seg_mask_downsample, edge_mask_downsample,
                            point_mask_downsample):
        # #print("forward ",self.layers_up2[0])
        point_mask_upsample = []
        for inx in range(len(self.layers_up)):
            if inx == 0:
                point_mask = self.layers_up3[inx](point_mask)
                ...
                point_mask = torch.cat([point_mask, point_mask_downsample[3 - inx]], -1)
                point_mask = self.concat_back_dim3[inx](point_mask)
                point_mask, list_out_q = self.layers_up3[inx](point_mask)
                # print("list_out_q", list_out_q)
                ...
                seg_mask, _ = self.layers_up[inx](seg_mask, list_cross_q=list_out_q)


                edge_mask = torch.cat([edge_mask, edge_mask_downsample[3 - inx]], -1)
                edge_mask = self.concat_back_dim2[inx](edge_mask)
                edge_mask, _ = self.layers_up2[inx](edge_mask, list_cross_q=list_out_q)



        seg_mask = self.norm_up(seg_mask)
        edge_mask = self.norm_up2(edge_mask)  # B L C
        point_mask = self.norm_up3(point_mask)

        return seg_mask, edge_mask, point_mask

    def up_x4(self, seg_mask, edge_mask, point_mask):
        ...
        if self.final_upsample == "expand_first":
            ...
            # point_mask = self.up3(point_mask)
            # print("point_mask1",point_mask.shape)
            # point_mask = point_mask.view(B, 4 * H, 4 * W, -1)
            point_mask = point_mask.view(B, H, W, -1)
            point_mask = point_mask.permute(0, 3, 1, 2)  # B,C,H,W
            # print("point_mask", point_mask.shape)
            point_mask = self.output3(point_mask)
        # #print("up_x4 x size ",x.shape)
        return seg_mask, edge_mask, point_mask

    def forward(self, x):
        seg_mask, edge_mask, point_mask, seg_mask_downsample, edge_mask_downsample, point_mask_downsample = self.forward_features(
            x)

        # #print("downsampling, ")

        seg_mask, edge_mask, point_mask = self.forward_up_features(seg_mask, edge_mask, point_mask, seg_mask_downsample,
                                                                   edge_mask_downsample, point_mask_downsample)
        # #print("forward features ", seg_mask.shape,edge_mask.shape,cluster_edge.shape)

        seg_mask, edge_mask, point_mask = self.up_x4(seg_mask, edge_mask, point_mask)
        # print("seg_mask", seg_mask.shape, edge_mask.shape, point_mask.shape)
...
#     model = TransNuSeg(img_size=256)
#     # output=model(img)
#     # summary(model, torch.zeros((1, 3, 256, 256)))
#     c1, c2, c3 = model(torch.zeros((1, 3, 256, 256)))
#     print("111", c1.size(), c2.size(), c3.size())
